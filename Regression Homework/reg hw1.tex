...
\begin{homeworkProblem}     %8
    ...
    \[\text{Firstly we get the observation data. Y is number of active physicians and X is the combination of the}\]
    \text{three predictor variables.}
    \begin{verbatim}
...
\begin{homeworkProblem}     %9
    ...
    \[
        ...
        &Y=\beta_0+\beta_1X+\beta_2R1+\beta_3R2+\beta_3R3+\varepsilon\\
        &\text{If there is no region effect, then the reduced model is}\\
        &Y=\beta_0+\beta_1X+\varepsilon\\
        &\text{for the full model,we have}\\
        \end{split}
    \]
        \begin{verbatim}
        ...
        \text{~~~~~~~~~~~~For the reduced model,we have}
        \begin{verbatim}
        ...
        Thus\[F^*=\frac{(3735858256-3496250017)/(438-432)}{3496250017/432}\]
    ...
    \begin{verbatim}
        ...
        [1] 8214318
    \end{verbatim}
    \text{ from the results above, we can conclude that the variable around the fitted regression line approximately}\\
    \text{the same for the region \#1,2,4. But the variable for region \#3 is a little larger than the other three. }
\end{homeworkProblem}
